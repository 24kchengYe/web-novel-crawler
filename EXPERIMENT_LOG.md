...
| 数据集 | 状态 | 样本数 | 汉字量 |
|--------|------|--------|--------|
| webnovel_cn | **complete** | 50,000 | 2,642 万 |
| chinese-novel-dataset | **complete** | 3,862 | 140 万 |
| GuoFeng-Webnovel | downloading | — | — |
| Chinese-Pixiv-Novel | pending (需手动下载 12.9GB) | 145K | — |
| LongData-Corpus | pending (需手动从清华云盘下载) | — | — |

**站点爬取探测**：

| 站点 | 可达性 | 书单解析 | 章节列表 | 正文获取 | 结论 |
|------|--------|---------|---------|---------|------|
| quanben-xiaoshuo.com | OK (1.7s) | OK (200本/页) | **FAIL** (JS 渲染, Playwright 也无法获取) | — | **放弃** |
| quanben.io | OK (2.7s) | OK | **FAIL** (JS 渲染) | — | **放弃** |
| xbiquge.com.cn | OK (11s, 慢) | OK (20本/页) | **OK** (urllib) | **OK** (Playwright 5s 等待) | **可行** |
| 69shuba.com | FAIL (SSL 超时) | — | — | — | 需代理 |
| dingdiann.com | FAIL (超时) | — | — | — | 需代理 |

**关键发现**：
- 2026 年主流盗版小说站已普遍采用 JS 动态加载正文，纯 HTTP 请求无法获取
- quanben 系列站甚至章节列表都是 JS 加载，完全无法静态爬取
- biquge 章节列表可静态解析，正文需 Playwright（headless Chrome 等待 5s）
- Playwright 单线程每章约 6 秒，一本 500 章的书约需 50 分钟

**biquge Playwright 爬取**（进行中）：
- 8 分类 × 50 本/类 = 最多 400 本
- 单线程运行，Monitor 跟踪

#### Analysis

- **Established**: 纯 HTTP 爬虫在 2026 年的盗版小说站上已大幅失效，JS 渲染是标配反爬手段
- **Established**: Playwright headless 模式可突破 biquge 的 JS 反爬，正文获取验证通过（3 章测试）
- **Constraint**: quanben 系列站完全无法爬取（JS 渲染+无 API 暴露），必须放弃
- **Constraint**: Playwright 单线程瓶颈严重（~6s/章），全量爬取耗时长
- **Hypothesis**: 69shu/顶点可能通过代理+Playwright 可行，但优先级降低

→ 策略调整：不等新站点全量完成，用现有 29.42 亿字 + 5 万条开源数据先启动训练（EXP-4.1/4.2），biquge 爬取后台继续

#### Artifacts

- 下载器：`scraper/download_datasets.py`
- 爬虫框架：`scraper/sites/base.py`
- Playwright 获取器：`scraper/sites/playwright_fetcher.py`
- 站点模块：`scraper/sites/quanben.py`、`biquge.py`、`shu69.py`、`dingdian.py`
- 驱动脚本：`scraper/crawl_all.py`
- 开源数据输出：`data_opensource/`（webnovel_cn + chinese-novel-dataset）
- biquge 数据输出：`data_biquge/`（爬取中）
...
| 路径 | 说明 |
|------|------|
| `data/` | 原始爬取数据（33GB，不修改） |
| `data_cleaned/` | 清洗后数据 |
| `data_cleaned/cleaning_report.json` | 清洗汇总统计 |
| `booklist.json` | 总书单（1959 本） |
| `jjwxc_books.json` | 晋江书目数据 |

### B. 代码索引

| 模块 | 文件 | 说明 |
|------|------|------|
| 爬虫 | `scraper/novel_scraper.py` | 核心爬虫（qbxsw.com） |
